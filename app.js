...
function validarNome(t) { return String(t).trim().length >= 2 }
...
async function processarMensagemWhatsApp(numero, texto) {
  ...
  if (COMANDOS_REINICIO.includes(tn)) {
    ...
    await enviarMensagemWhatsApp(numero, '🔄 Atendimento reiniciado.\n\nPara começar, envie:\n*Olá, quero conhecer a clínica.*')
    return
  }
  ...
  const respostaFinal = etapaAtual.opcoes ? interpretarOpcao(etapaAtual, texto) : texto
  const respostasAtualizadas = { ...(sessao.respostas || {}), [etapaAtual.campo]: respostaFinal }
  const novaEtapa = sessao.etapa + 1

  await atualizarSessao(numero, { etapa: novaEtapa, respostas: respostasAtualizadas, ativo: novaEtapa < fluxo.length })
...
function gerarDashboard(leads, busca) {
  ...
  const leadsFiltrados = termo
    ? leads.filter(l => [l.nome, l.phone, l.idade, l.dorPrincipal, l.objetivoAtual, formatarData(l.createdAt)].join(' ').toLowerCase().includes(termo))
    : leads
  ...
  const linhas = leadsFiltrados.map(l => {
    ...
    return `
    <tr>
      <td><div class="lead-name">${escHtml(l.nome || '-')}</div><div class="lead-phone">${escHtml(l.phone || '-')}</div></td>
...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  :root{--bg:#f5f5f7;--surface:#fff;--surface2:#f5f5f7;--border:rgba(0,0,0,.08);--border2:rgba(0,0,0,.12);--text:#1d1d1f;--text2:#6e6e73;--text3:#aeaeb2;--accent:#0071e3;--r-lg:16px;--sh-sm:0 1px 3px rgba(0,0,0,.06);--sh-md:0 4px 16px rgba(0,0,0,.08);--font:'Inter',-apple-system,sans-serif}
  body{font-family:var(--font);background:var(--bg);color:var(--text);min-height:100vh;-webkit-font-smoothing:antialiased}
  .topbar{background:rgba(255,255,255,.85);backdrop-filter:blur(20px);border-bottom:1px solid var(--border);position:sticky;top:0;z-index:100;padding:0 32px;height:56px;display:flex;align-items:center;justify-content:space-between}
  ...
  .refresh-btn{padding:6px 14px;background:var(--surface);border:1px solid var(--border2);border-radius:20px;font-size:13px;font-weight:500;color:var(--text);text-decoration:none}
  ...
  .metric-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--r-lg);padding:20px 22px;box-shadow:var(--sh-sm)}
  ...
  .metric-value{font-size:32px;font-weight:600;line-height:1;margin-bottom:6px}
  ...
  .busca-input{flex:1;max-width:360px;padding:10px 16px;border:1px solid var(--border2);border-radius:20px;font-size:14px;font-family:var(--font);background:var(--surface);color:var(--text);outline:none}
  ...
  thead th{padding:10px 16px;text-align:left;font-size:11px;font-weight:600;color:var(--text2);text-transform:uppercase;background:var(--surface2);border-bottom:1px solid var(--border)}
  ...
  @media(max-width:600px){.page{padding:20px}.metrics{grid-template-columns:1fr 1fr}}
</style>
</head>
<body>
<nav class="topbar">
  <div class="topbar-brand">
    <div class="topbar-icon"><svg width="16" height="16" viewBox="0 0 18 18" fill="none"><circle cx="9" cy="6" r="3.5" fill="white"/><path d="M2 15c0-3.866 3.134-6 7-6s7 2.134 7 6" stroke="white" stroke-width="1.6" stroke-linecap="round"/></svg></div>
...
<div class="page">
  <div class="page-header"><h1>Dashboard de Leads</h1><p>Acompanhe os pré-atendimentos em tempo real.</p></div>
  <div class="metrics">
    <div class="metric-card accent"><div class="metric-label">Total de leads</div><div class="metric-value">${totais.total}</div><div class="metric-sub">captados até agora</div></div>
    <div class="metric-card"><div class="metric-label">Queixa mais comum</div><div class="metric-value" style="font-size:20px;line-height:1.2">${escHtml(dorMaisComum)}</div><div class="metric-sub">mais recorrente</div></div>
    <div class="metric-card"><div class="metric-label">Objetivo principal</div><div class="metric-value" style="font-size:20px;line-height:1.2">${escHtml(objMaisComum)}</div><div class="metric-sub">mais buscado</div></div>
    <div class="metric-card"><div class="metric-label">Novos leads</div><div class="metric-value" style="color:#92400e">${totais.novo}</div><div class="metric-sub">aguardando contato</div></div>
    <div class="metric-card"><div class="metric-label">Contato feito</div><div class="metric-value" style="color:#1d4ed8">${totais.contatoFeito}</div><div class="metric-sub">em andamento</div></div>
    <div class="metric-card"><div class="metric-label">Agendados</div><div class="metric-value" style="color:#166534">${totais.agendado}</div><div class="metric-sub">consulta marcada</div></div>
  </div>
  ...
  <div class="table-card">
    ...
    <div class="table-scroll">
      <table>
        <thead><tr><th>Contato</th><th>Idade</th><th>Queixa principal</th><th>Intensidade</th><th>Objetivo</th><th>Status</th><th>Detalhes</th></tr></thead>
        <tbody>${linhas}</tbody>
      </table>
    </div>`}
  </div>
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST' && urlPath === '/webhook') {
      // Lê o body ANTES de responder (stream só pode ser lido uma vez)
      let payload = {}
      try {
        ...
        try {
          if (EVOLUTION_WEBHOOK_TOKEN) {
            ...
            if (tokenRecebido !== EVOLUTION_WEBHOOK_TOKEN) {
              console.warn('⚠️ Webhook com token inválido, ignorando.')
              return
            }
          }

          console.log('📩 Webhook recebido, event:', payload?.event)
    ...
    if (urlPath === '/api/status') { responderJson(res, 200, { status: botState.status }); return }
    if (urlPath === '/api/leads') { const leads = await listarLeads(); responderJson(res, 200, leads); return }
...
server.listen(PORT, () => {
  ...
  // Log das variáveis no startup para diagnóstico
  console.log('🔧 EVOLUTION_API_URL:', process.env.EVOLUTION_API_URL || '❌ NÃO DEFINIDA')
  console.log('🔧 EVOLUTION_INSTANCE:', process.env.EVOLUTION_INSTANCE || '❌ NÃO DEFINIDA')
  console.log('🔧 EVOLUTION_API_KEY:', process.env.EVOLUTION_API_KEY ? '✅ definida' : '❌ NÃO DEFINIDA')
  console.log('🔧 EVOLUTION_INSTANCE_TOKEN:', process.env.EVOLUTION_INSTANCE_TOKEN ? '✅ definida' : '❌ NÃO DEFINIDA')
})
